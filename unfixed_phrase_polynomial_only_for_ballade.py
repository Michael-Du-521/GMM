...
currently_processed_music_name= music_name4

#polynomial_degree for fitting
polynomial_degree= 12
# tempo calculation and then smoothing
df_1_tempo_BPM_revise_smoothing_list=readFileThenSmoothing(fileName=fileName4)
...
for k in df_1_tempo_BPM_revise_smoothing_standardization_dic.keys():
    for i in range(1, 23):
        ...
        phrases_segmentation_dic[k].append(df_1_tempo_BPM_revise_smoothing_standardization_dic[k][i - 1][1:])
...
for k in df_1_tempo_BPM_revise_smoothing_standardization_dic.keys():
    for i in range(1,23):
        fig, axes = plt.subplots(2, int(len(phrase_segmentation_annotation_list_dic[currently_processed_music_name])/2))
        for l,ax in zip(range(0,len(df_1_tempo_BPM_revise_smoothing_standardization_dic[k][i - 1][1:])),axes.flat):# l is the x th phrase
            X = array([j + 1 for j in range(0,len(df_1_tempo_BPM_revise_smoothing_standardization_dic[k][i - 1][1:][l]))]).reshape(-1,1)
            y = df_1_tempo_BPM_revise_smoothing_standardization_dic[k][i-1][1:][l]
...
for i,(k, clf_list) in enumerate(best_gmm_each_standard_dic.items()):
    ...
    heat_map = sns.heatmap(Y_separate, linewidth=1, annot=True, yticklabels=Pianist_index, xticklabels=[i for i in range(1, len(phrase_segmentation_annotation_list_dic[currently_processed_music_name]))], ax=spl1)
    ...
    Y_=np.reshape(Y_, (22,len(phrase_segmentation_annotation_list_dic[currently_processed_music_name][1:])))
    Y_=Y_.T
    for i in range(len(phrase_segmentation_annotation_list_dic[currently_processed_music_name][1:])):
    ...
    Y_=np.array(Most_frequent_pharse_prediction_dic[k]*22).reshape(22, len(phrase_segmentation_annotation_list_dic[currently_processed_music_name][1:]))
...
for i,k in enumerate(best_gmm_each_standard_dic.keys()):
    ...
    Pianist_index= [i for i in range(1,23)]
    plt.style.use("seaborn")
    heat_map = sns.heatmap(Most_frequent_pharse_prediction_dic[k], linewidth=1, annot=True, yticklabels=Pianist_index, xticklabels=[i for i in range(1, len(phrase_segmentation_annotation_list_dic[currently_processed_music_name]))], ax=spl2)
    heat_map.invert_yaxis()
